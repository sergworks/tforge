...
uses
  ...
  {$IFDEF TFL_DLL}
    tfImport
  {$ELSE}
    tfAESGetter, tfDES, tfRC4, tfRC5, tfSalsa20, tfCipherServ, // tfKeyStreams,
    tfEvpAES, tfOpenSSL
  {$ENDIF};
...
const
  ...
  ALG_DES      = TF_ALG_DES;
  ALG_RC5      = TF_ALG_RC5;
  ALG_DES3     = TF_ALG_3DES;

  ALG_RC4      = TF_ALG_RC4;
  ALG_SALSA    = TF_ALG_SALSA20;
  ALG_CHACHA   = TF_ALG_CHACHA20;
  ...
  ENCRYPT_ECB  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_ECB;
  DECRYPT_ECB  = TF_KEYDIR_DECRYPT or TF_KEYMODE_ECB;

  ENCRYPT_CBC  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_CBC;
  DECRYPT_CBC  = TF_KEYDIR_DECRYPT or TF_KEYMODE_CBC;

  ENCRYPT_CFB  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_CFB;
  DECRYPT_CFB  = TF_KEYDIR_DECRYPT or TF_KEYMODE_CFB;

  ENCRYPT_OFB  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_OFB;
  DECRYPT_OFB  = TF_KEYDIR_DECRYPT or TF_KEYMODE_OFB;

  ENCRYPT_CTR  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_CTR;
  DECRYPT_CTR  = TF_KEYDIR_DECRYPT or TF_KEYMODE_CTR;

  ENCRYPT_GCM  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_GCM;
  DECRYPT_GCM  = TF_KEYDIR_DECRYPT or TF_KEYMODE_GCM;

  ECB_ENCRYPT  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_ECB deprecated;
  ECB_DECRYPT  = TF_KEYDIR_DECRYPT or TF_KEYMODE_ECB deprecated;

  CBC_ENCRYPT  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_CBC deprecated;
  CBC_DECRYPT  = TF_KEYDIR_DECRYPT or TF_KEYMODE_CBC deprecated;

  CTR_ENCRYPT  = TF_KEYDIR_ENCRYPT or TF_KEYMODE_CTR deprecated;
  CTR_DECRYPT  = TF_KEYDIR_DECRYPT or TF_KEYMODE_CTR deprecated;

  AES_ECB         = ALG_AES or MODE_ECB;
  AES_ENCRYPT_ECB = ALG_AES or ENCRYPT_ECB;
  AES_DECRYPT_ECB = ALG_AES or DECRYPT_ECB;
  AES_CBC         = ALG_AES or MODE_CBC;
  AES_ENCRYPT_CBC = ALG_AES or ENCRYPT_CBC;
  AES_DECRYPT_CBC = ALG_AES or DECRYPT_CBC;
  AES_CFB         = ALG_AES or MODE_CFB;
  AES_ENCRYPT_CFB = ALG_AES or ENCRYPT_CFB;
  AES_DECRYPT_CFB = ALG_AES or DECRYPT_CFB;
  AES_OFB         = ALG_AES or MODE_OFB;
  AES_ENCRYPT_OFB = ALG_AES or ENCRYPT_OFB;
  AES_DECRYPT_OFB = ALG_AES or DECRYPT_OFB;
  AES_CTR         = ALG_AES or MODE_CTR;
  AES_ENCRYPT_CTR = ALG_AES or ENCRYPT_CTR;
  AES_DECRYPT_CTR = ALG_AES or DECRYPT_CTR;
  AES_GCM         = ALG_AES or MODE_GCM;
  AES_ENCRYPT_GCM = ALG_AES or ENCRYPT_GCM;
  AES_DECRYPT_GCM = ALG_AES or DECRYPT_GCM;

  DES_ECB         = ALG_DES or MODE_ECB;
  DES_ENCRYPT_ECB = ALG_DES or ENCRYPT_ECB;
  DES_DECRYPT_ECB = ALG_DES or DECRYPT_ECB;
  DES_CBC         = ALG_DES or MODE_CBC;
  DES_ENCRYPT_CBC = ALG_DES or ENCRYPT_CBC;
  DES_DECRYPT_CBC = ALG_DES or DECRYPT_CBC;
  DES_CFB         = ALG_DES or MODE_CFB;
  DES_ENCRYPT_CFB = ALG_DES or ENCRYPT_CFB;
  DES_DECRYPT_CFB = ALG_DES or DECRYPT_CFB;
  DES_OFB         = ALG_DES or MODE_OFB;
  DES_ENCRYPT_OFB = ALG_DES or ENCRYPT_OFB;
  DES_DECRYPT_OFB = ALG_DES or DECRYPT_OFB;
  DES_CTR         = ALG_DES or MODE_CTR;
  DES_ENCRYPT_CTR = ALG_DES or ENCRYPT_CTR;
  DES_DECRYPT_CTR = ALG_DES or DECRYPT_CTR;

  DES3_ECB         = ALG_DES3 or MODE_ECB;
  DES3_ENCRYPT_ECB = ALG_DES3 or ENCRYPT_ECB;
  DES3_DECRYPT_ECB = ALG_DES3 or DECRYPT_ECB;
  DES3_CBC         = ALG_DES3 or MODE_CBC;
  DES3_ENCRYPT_CBC = ALG_DES3 or ENCRYPT_CBC;
  DES3_DECRYPT_CBC = ALG_DES3 or DECRYPT_CBC;
  DES3_CTR         = ALG_DES3 or MODE_CTR;
  DES3_ENCRYPT_CTR = ALG_DES3 or ENCRYPT_CTR;
  DES3_DECRYPT_CTR = ALG_DES3 or DECRYPT_CTR;

  RC5_ECB         = ALG_RC5 or MODE_ECB;
  RC5_ENCRYPT_ECB = ALG_RC5 or ENCRYPT_ECB;
  RC5_DECRYPT_ECB = ALG_RC5 or DECRYPT_ECB;
  RC5_CBC         = ALG_RC5 or MODE_CBC;
  RC5_ENCRYPT_CBC = ALG_RC5 or ENCRYPT_CBC;
  RC5_DECRYPT_CBC = ALG_RC5 or DECRYPT_CBC;
  RC5_CTR         = ALG_RC5 or MODE_CTR;
  RC5_ENCRYPT_CTR = ALG_RC5 or ENCRYPT_CTR;
  RC5_DECRYPT_CTR = ALG_RC5 or DECRYPT_CTR;
...

{
    ...
    function Init(AKey: PByte; AKeyLen: Cardinal): TCipher; overload;
    function Init(const AKey: ByteArray): TCipher; overload;
    function Init(AKey: PByte; AKeyLen: Cardinal;
                       AIV: Pointer; AIVLen: Cardinal): TCipher; overload;
    function Init(const AKey: ByteArray;
    ...
    function EncryptInit(AKey: PByte; AKeyLen: Cardinal): TCipher; overload;
    function EncryptInit(const AKey: ByteArray): TCipher; overload;
    function EncryptInit(AKey: PByte; AKeyLen: Cardinal;
                       AIV: Pointer; AIVLen: Cardinal): TCipher; overload;
    function EncryptInit(const AKey: ByteArray;
                       const AIV: ByteArray): TCipher; overload;
    function EncryptInit(AKey: PByte; AKeyLen: Cardinal;
                       const ANonce: UInt64): TCipher; overload;
    function EncryptInit(const AKey: ByteArray;
    ...
    function DecryptInit(AKey: PByte; AKeyLen: Cardinal): TCipher; overload;
    function DecryptInit(const AKey: ByteArray): TCipher; overload;
    function DecryptInit(AKey: PByte; AKeyLen: Cardinal;
                       AIV: Pointer; AIVLen: Cardinal): TCipher; overload;
    function DecryptInit(const AKey: ByteArray;
                       const AIV: ByteArray): TCipher; overload;
    function DecryptInit(AKey: PByte; AKeyLen: Cardinal;
                       const ANonce: TNonce): TCipher; overload;
    function DecryptInit(const AKey: ByteArray;
                       const ANonce: TNonce): TCipher; overload;
{
// key + IV
...
// key only (zeroed IV)
    ...
    procedure EncryptUpdate(InBuffer, OutBuffer: PByte; var DataSize: Cardinal;
    ...
    procedure AddAuthData(Data: Pointer; DataSize: Cardinal); overload;
    procedure AddAuthData(const Data: ByteArray); overload;

    procedure GetAuthTag(Data: Pointer; TagSize: Cardinal); overload;
    function GetAuthTag(TagSize: Cardinal): ByteArray; overload;

    function CheckAuthTag(Data: Pointer; TagSize: Cardinal): Boolean; overload;
    function CheckAuthTag(const Tag: ByteArray): Boolean; overload;
...
//    class function DecryptBlock(AlgID: TAlgID; const Data, Key: ByteArray): ByteArray; static;
    ...
    function DecryptStream(InStream, OutStream: TStream; BufSize: Cardinal = 0): TCipher;

    procedure EncryptFile(const InName, OutName: string; BufSize: Cardinal = 0);
    function DecryptFile(const InName, OutName: string; BufSize: Cardinal = 0): TCipher;
...
//    function Skip(Value: UInt32): TCipher; overload;
    ...
    class function GetAESInstance(AlgID: TAlgID): TCipher; static;
...
function TCipher.Init(AKey: PByte; AKeyLen: Cardinal; AIV: Pointer;
  AIVLen: Cardinal): TCipher;
begin
...
function TCipher.Init(const AKey: ByteArray): TCipher;
begin
...
function TCipher.Init(AKey: PByte; AKeyLen: Cardinal): TCipher;
begin
...
function TCipher.Init(const AKey: ByteArray; const ANonce: UInt64): TCipher;
begin
...
function TCipher.Init(AKey: PByte; AKeyLen: Cardinal; const ANonce: UInt64): TCipher;
begin
...
function TCipher.Init(const AKey, AIV: ByteArray): TCipher;
begin
...
begin
  HResCheck(GetCipherInstance(Result.FInstance, AlgID));
end;

// the purpose of GetXXXInstance methods is to support smart linking;
//   if you need only AES and don't want other algorithms in the executable file,
//   use GetAESInstance instead of GetInstance

class function TCipher.GetAESInstance(AlgID: TAlgID): TCipher;
begin
  HResCheck(GetAESCipherInstance(Pointer(Result.FInstance), AlgID));
end;
...
function TCipher.ExpandKey(AKey: PByte; AKeyLen: Cardinal;
...
function TCipher.ExpandKey(const AKey: ByteArray;
...
procedure TCipher.AddAuthData(Data: Pointer; DataSize: Cardinal);
begin
  HResCheck(FInstance.AddAuthData(Data, DataSize));
end;

procedure TCipher.AddAuthData(const Data: ByteArray);
begin
  AddAuthData(Data.Raw, Data.Len);
end;

procedure TCipher.GetAuthTag(Data: Pointer; TagSize: Cardinal);
begin
  HResCheck(FInstance.GetAuthTag(Data, TagSize));
end;

function TCipher.GetAuthTag(TagSize: Cardinal): ByteArray;
begin
  Result:= ByteArray.Allocate(TagSize);
  GetAuthTag(Result.Raw, TagSize);
end;

function TCipher.CheckAuthTag(Data: Pointer; TagSize: Cardinal): Boolean;
begin
  Result:= FInstance.CheckAuthTag(Data, TagSize) = TF_S_OK;
end;

function TCipher.CheckAuthTag(const Tag: ByteArray): Boolean;
begin
  Result:= CheckAuthTag(Tag.Raw, Tag.Len);
end;
...
procedure TCipher.EncryptUpdate(InBuffer, OutBuffer: PByte; var DataSize: Cardinal;
            OutBufSize: Cardinal; Last: Boolean);
begin
  HResCheck(FInstance.EncryptUpdate(InBuffer, OutBuffer, DataSize, OutBufSize, Last));
end;

procedure TCipher.DecryptUpdate(InBuffer, OutBuffer: PByte; var DataSize: Cardinal;
            OutBufSize: Cardinal; Last: Boolean);
begin
...
begin
  ...
  Cipher:= TCipher.GetInstance(AlgID or ENCRYPT_ECB).Init(Key);
...
begin
  AlgID:= AlgID and TF_ALGID_MASK;
  Cipher:= TCipher.GetInstance(AlgID or DECRYPT_ECB).Init(Key);
  BlockSize:= Cipher.GetBlockSize;
  if (BlockSize = 0) or (BlockSize > TF_MAX_CIPHER_BLOCK_SIZE) then
    CipherError(TF_E_UNEXPECTED);
  if (BlockSize <> Data.GetLen) then
...
// one LBlockSize for cache dump, another for pad block
  ...
  try
    ...
    HResCheck(FInstance.EncryptUpdate(InBuffer, OutBuffer, L0, L1, True));
...
// LBlockSize for cache dump
  ...
  try
    InBuffer:= OutBuffer + LBlockSize;
    Move(Data.RawData^, InBuffer^, L0);
    HResCheck(FInstance.DecryptUpdate(InBuffer, OutBuffer, L0, L1, True));
    Result:= ByteArray.FromMem(OutBuffer, L0);
  finally
    FillChar(OutBuffer^, L1, 0);
    FreeMem(OutBuffer);
...
function TCipher.EncryptInit(AKey: PByte; AKeyLen: Cardinal; AIV: Pointer;
  AIVLen: Cardinal): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_ENCRYPT));
  HResCheck(FInstance.ExpandKeyIV(AKey, AKeyLen, AIV, AIVLen));
//  Result:= Self;
  Result.FInstance:= FInstance;
end;

function TCipher.EncryptInit(const AKey: ByteArray): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_ENCRYPT));
  HResCheck(FInstance.ExpandKey(AKey.RawData, AKey.Len));
//  Result:= Self;
...
function TCipher.EncryptInit(AKey: PByte; AKeyLen: Cardinal): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_ENCRYPT));
  HResCheck(FInstance.ExpandKey(AKey, AKeyLen));
//  Result:= Self;
...
function TCipher.EncryptInit(const AKey: ByteArray;
  const ANonce: UInt64): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_ENCRYPT));
  HResCheck(FInstance.ExpandKeyNonce(AKey.RawData, AKey.Len, ANonce));
...
function TCipher.EncryptInit(AKey: PByte; AKeyLen: Cardinal;
  const ANonce: UInt64): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_ENCRYPT));
  HResCheck(FInstance.ExpandKeyNonce(AKey, AKeyLen, ANonce));
...
function TCipher.EncryptInit(const AKey, AIV: ByteArray): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_ENCRYPT));
  HResCheck(FInstance.ExpandKeyIV(AKey.RawData, AKey.Len, AIV.RawData, AIV.Len));
//  Result:= Self;
...
{$ENDIF}
  ...
  try
    repeat
      ...
      EncryptUpdate(InBuffer, OutBuffer, DataSize, OutBufSize, Last);
      if DataSize > 0 then
...
function TCipher.DecryptStream(InStream, OutStream: TStream; BufSize: Cardinal): TCipher;
const
  MIN_BUFSIZE = 4 * 1024;
  MAX_BUFSIZE = 4 * 1024 * 1024;
  DEFAULT_BUFSIZE = 16 * 1024;
...
{$ENDIF}
  ...
  try
    repeat
      ...
      Last:= Cnt > 0;
      DataSize:= BufSize - Cnt;
      DecryptUpdate(InBuffer, OutBuffer, DataSize, OutBufSize, Last);
      if DataSize > 0 then
        OutStream.WriteBuffer(OutBuffer^, DataSize);
    until Last;
//  Result:= Self;
...
function TCipher.DecryptInit(AKey: PByte; AKeyLen: Cardinal; AIV: Pointer;
  AIVLen: Cardinal): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_DECRYPT));
  HResCheck(FInstance.ExpandKeyIV(AKey, AKeyLen, AIV, AIVLen));
//  Result:= Self;
  Result.FInstance:= FInstance;
end;

function TCipher.DecryptInit(const AKey: ByteArray): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_DECRYPT));
  HResCheck(FInstance.ExpandKey(AKey.RawData, AKey.Len));
//  Result:= Self;
...
function TCipher.DecryptInit(AKey: PByte; AKeyLen: Cardinal): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_DECRYPT));
  HResCheck(FInstance.ExpandKey(AKey, AKeyLen));
//  Result:= Self;
...
function TCipher.DecryptInit(const AKey: ByteArray; const ANonce: TNonce): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_DECRYPT));
  HResCheck(FInstance.ExpandKeyNonce(AKey.RawData, AKey.Len, ANonce));
//  Result:= Self;
...
function TCipher.DecryptInit(AKey: PByte; AKeyLen: Cardinal; const ANonce: TNonce): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_DECRYPT));
  HResCheck(FInstance.ExpandKeyNonce(AKey, AKeyLen, ANonce));
//  Result:= Self;
...
function TCipher.DecryptInit(const AKey, AIV: ByteArray): TCipher;
begin
  HResCheck(FInstance.SetKeyDir(TF_KEYDIR_DECRYPT));
  HResCheck(FInstance.ExpandKeyIV(AKey.RawData, AKey.Len, AIV.RawData, AIV.Len));
//  Result:= Self;
