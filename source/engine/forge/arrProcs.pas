...
{ *********************************************************** }
{
  ...
  The return value is stored in the rax register,
    or if it is a 128-bit value, then the higher 64-bits go in rdx.
}
...
{$IFDEF TFL_CPU386_WIN32_LIMB32}
  {$DEFINE ASM86}
{$ENDIF}

{$IFDEF TFL_CPUX64_WIN64_LIMB32}
  {$DEFINE ASM64}
{$ENDIF}

{$IFDEF TFL_CPUX64_LINUX64_LIMB32}
  {$DEFINE ASM64_LINUX}
{$ENDIF}
...
function arrAddEx(A, B, Res: PLimb; LA, LB: Cardinal): Integer;
...

{
  ...
  Remarks:
    ...
    if function returns False the Res senior limb is cleared: Res[LA] = 0
}
{$IFDEF ASM86}
function arrAdd(A, B, Res: PLimb; LA, LB: Cardinal): Boolean;
asm
...
{$ELSE}
{$IFDEF ASM64}
// RCX <- A, RDX <- B, R8 <- Res, R9D <- LA
function arrAdd(A, B, Res: PLimb; LA, LB: Cardinal): Boolean;
asm
        MOV   R10,RCX       // R10 <-- A
        MOV   ECX,LB        // ECX <-- LB
        SUB   R9D,ECX       // R9D <-- LA - LB
        CLC
@@Loop:
        MOV   EAX,[R10]
        LEA   R10,[R10+4]
...
{$ELSE}
{$IFDEF ASM64_LINUX}
// RDI <- A, RSI <- B, RDX <- Res, ECX <- LA, R8D <- LB
function arrAdd(A, B, Res: PLimb; LA, LB: Cardinal): Boolean;
asm
        SUB   ECX,R8D        // ECX <-- LA - LB
        CLC                  // ? not needed because LA >= LB
@@Loop:
        MOV   EAX,[RDI]
        LEA   RDI,[RDI+4]
        ADC   EAX,[RSI]
...

{
  ...
  Asserts:
    ...
    Res must have enough space for LA + 1 limbs
  Remarks:
    function returns number of limbs in Res
}
{$IFDEF ASM86}
function arrAddEx(A, B, Res: PLimb; LA, LB: Cardinal): Integer;
asm
...
{$ELSE}
{$IFDEF ASM64_LINUX}
// RDI <- A, ESI <- Limb, RDX <- Res, ECX <- LA
function arrAddLimb(A: PLimb; Limb: TLimb; Res: PLimb; LA: Cardinal): Boolean;
{$IFDEF FPC}assembler; nostackframe;{$ENDIF}
asm
        ADD   ESI,[RDI]     // Limb:= Limb + A[0]
        MOV   [RDX],ESI
        DEC   ECX
        JZ    @@Done
@@Loop:
...
{$ELSE}
{$IFDEF ASM64_LINUX}
// RDI <- A, RSI <- B, EDX <- LA, ECX <- LB
function arrSelfAdd(A, B: PLimb; LA, LB: Cardinal): Boolean;
asm
        SUB   EDX,ECX     // LA <-- LA - LB
        CLC
@@Loop:
        MOV   EAX,[RDI]
        ADC   [RSI],EAX
...
function arrMul(A, B, Res: PLimb; LA, LB: Cardinal): Boolean;
asm
        PUSH  RBX           // EBX stores Carry digit
        PUSH  RBP           // EBP stores Multiplier digit
        PUSH  R12

        MOV   R10,RDI       // R10 stores pointer to A
        MOV   R11,RDX       // R11 stores moving pointer to Res

        MOV   R9D,ECX       // R9D stores LA; ECX is used as counter
                            // clear first LA digits of Res
        XOR   EAX,EAX
@@Clear:                    
        MOV   [RDX],EAX
        LEA   RDX,[RDX+4]
...
@@ExtLoop:
        XOR   EBX,EBX       // Carry Limb <- 0
        MOV   EBP,[RSI]     // Multiplier Limb <- B^
        MOV   ECX,R9D       // TmpLA  <- LA
        MOV   RDI,R10       // TmpA   <- A
        MOV   R12,R11       // TmpRes <- Res

@@Loop:
        MOV   EAX,[RDI]     // EAX <- TmpA^
        LEA   RDI,[RDI+4]   // Inc(TmpA)
        MUL   EBP           // (EAX,EDX) <- EAX * EBP
        ADD   EAX,EBX       // (EAX,EDX) <- (EAX,EDX) + Carry Limb
        ADC   EDX,0
        ADD   EAX,[R12]     // (EAX,EDX) <- (EAX,EDX) + TmpRes^
        ADC   EDX,0
        MOV   [R12],EAX     // TmpRes^ <- EAX
        LEA   R12,[R12+4]   // Inc(TmpRes)
        MOV   EBX,EDX       // Carry Limb <- EDX
        LOOP  @@Loop

        MOV   [R12],EBX
        LEA   RSI,[RSI+4]   // Inc(B);
        LEA   R11,[R11+4]   // Inc(Res);
        DEC   R10D          // Dec(LB);
        JNZ   @@ExtLoop

        OR    EBX,EBX       // Result:= Carry Limb <> 0
        SETNZ AL
