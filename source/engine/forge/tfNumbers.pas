...
uses tfHelpers, tfInstances, tfUtils, arrProcs;
...
//        tfFreeInstance(R); //if R <> nil then TtfRecord.Release(R);
        R:= A;
        TForgeHelper.Addref(R); //TtfRecord.AddRef(R);
      end;
      Result:= TF_S_OK;
      Exit;
    end;
...
begin
  if A.IsZero or B.IsZero then begin
    TForgeHelper.Free(R); //if (R <> nil) then TtfRecord.Release(R);
    R:= @BigNumZero;
    Result:= TF_S_OK;
  end
  else begin
...
begin
  ...
  if Result = TF_S_OK then begin
    ...
    Tmp.FUsed:= Used;
    TForgeHelper.Free(Q); //if (Q <> nil) then TtfRecord.Release(Q);
    Q:= Tmp;
  end;
end;
...
// perform normalized division and shift the remaider right
  ...
  TForgeHelper.Release(Dividend); //TtfRecord.Release(Dividend);
  TForgeHelper.Release(Divisor);  //TtfRecord.Release(Divisor);
...
// divisor (B) has only 1 limb
  if (UsedB = 1) then begin
    ...
    if Result <> TF_S_OK then begin
      TForgeHelper.Release(Quotient); //TtfRecord.Release(Quotient);
      Exit;
    end;
...
// create normalized divisor by shifting the divisor B left
  ...
  Result:= AllocNumber(Divisor, UsedB);
  if Result <> TF_S_OK then begin
    TForgeHelper.Release(Remainder); //TtfRecord.Release(Remainder);
    TForgeHelper.Release(Quotient); //TtfRecord.Release(Quotient);
    Exit;
  end;
...
begin
{
  ...
  if Diff = 0 then begin
    ...
    TForgeHelper.Free(G); //if (G <> nil) then TtfRecord.Release(G);
    G:= A;
    TForgeHelper.Addref(A); //TtfRecord.Addref(A);
    Result:= TF_S_OK;
    Exit;
  end;
  ...
  TForgeHelper.Addref(TmpA); //TtfRecord.Addref(TmpA);
  ...
  TForgeHelper.Addref(TmpB); //TtfRecord.Addref(TmpB);
  TmpQ:= nil;
  TmpR:= nil;
...
// Q:= A div B, R:= A mod B;
    ...
    TForgeHelper.Release(TmpA); //TtfRecord.Release(TmpA);

    if Result <> TF_S_OK then begin
      TForgeHelper.Release(TmpB); //TtfRecord.Release(TmpB);
      TForgeHelper.Free(TmpQ); //if TmpQ <> nil then TtfRecord.Release(TmpQ);
      TForgeHelper.Free(TmpR); //if TmpR <> nil then TtfRecord.Release(TmpR);
      Exit;
    end;

    TForgeHelper.Release(TmpQ); //TtfRecord.Release(TmpQ);
    TmpQ:= nil;

    if TmpR.IsZero then begin
      TForgeHelper.Release(TmpR); //TtfRecord.Release(TmpR);
      TForgeHelper.Free(G); //if (G <> nil) then TtfRecord.Release(G);
      G:= TmpB;
      Exit;
    end;
...
begin
  if A.IsZero then begin
    if B.IsZero then begin
      ...
      TForgeHelper.Addref(B); //TtfRecord.AddRef(B);
      TForgeHelper.Free(G); //if (G <> nil) then TtfRecord.Release(G);
      G:= B;
      Result:= TF_S_OK;
    end;
  ...
  if Result <> TF_S_OK then begin
    TForgeHelper.Release(Tmp1); //TtfRecord.Release(Tmp1);
    Exit;
  end;

  TForgeHelper.Release(Tmp2); //TtfRecord.Release(Tmp2);
...
procedure CleanUp;
begin
  TForgeHelper.Release(A); //TtfRecord.Release(A);
  TForgeHelper.Release(B); //TtfRecord.Release(B);
  TForgeHelper.Free(TmpX); //if TmpX <> nil then TtfRecord.Release(TmpX);
  TForgeHelper.Free(TmpY); //if TmpY <> nil then TtfRecord.Release(TmpY);
  TForgeHelper.Free(TmpU); //if TmpU <> nil then TtfRecord.Release(TmpU);
  TForgeHelper.Free(TmpV); //if TmpV <> nil then TtfRecord.Release(TmpV);
  TForgeHelper.Free(TmpQ); //if TmpQ <> nil then TtfRecord.Release(TmpQ);
  TForgeHelper.Free(TmpR); //if TmpR <> nil then TtfRecord.Release(TmpR);
  TForgeHelper.Free(TmpM); //if TmpM <> nil then TtfRecord.Release(TmpM);
  TForgeHelper.Free(TmpN); //if TmpN <> nil then TtfRecord.Release(TmpN);
end;
...
//    TmpA.TtfRecord.Release(;
    ...
    TForgeHelper.Release(TmpX); //TtfRecord.Release(TmpX);
    TmpX:= TmpU;

    TForgeHelper.Release(TmpY); //TtfRecord.Release(TmpY);
    TmpY:= TmpV;
  ...
  TForgeHelper.Free(G); //if (G <> nil) then TtfRecord.Release(G);
  G:= B;
  TForgeHelper.Addref(G); //TtfRecord.Addref(G);

  TForgeHelper.Free(X); //if (X <> nil) then TtfRecord.Release(X);
  X:= TmpX;
  TForgeHelper.Addref(X); //TtfRecord.Addref(X);

  TForgeHelper.Free(Y); //if (Y <> nil) then TtfRecord.Release(Y);
  Y:= TmpY;
  TForgeHelper.Addref(Y); //TtfRecord.Addref(Y);
...
// if GCD <> 1
  if (TmpG.FUsed <> 1) or (TmpG.FLimbs[0] <> 1) then begin
    TForgeHelper.Release(TmpG); //TtfRecord.Release(TmpG);
    TForgeHelper.Release(TmpX); //TtfRecord.Release(TmpX);
    Result:= TF_E_INVALIDARG;
    Exit;
  end;
  ...
  TForgeHelper.Release(TmpQ); //TtfRecord.Release(TmpQ);
//  TForgeHelper.Free(TmpR);    //if (R <> nil) then TtfRecord.Release(TmpR);
  TForgeHelper.Free(R);    //if (R <> nil) then TtfRecord.Release(R);
  R:= TmpR;
end;
...
// to avoid negative zero
    ...
    TForgeHelper.Free(A); //if A <> nil then TtfRecord.Release(A);
    A:= Tmp;
  end;
end;
...
begin
  TForgeHelper.Free(R); //if R <> nil then TtfRecord.Release(R);
  R:= A;
  TForgeHelper.Addref(A); //if A <> nil then TtfRecord.AddRef(A);
  Result:= TF_S_OK;
end;
...
begin
  ...
  if Result = TF_S_OK then begin
    ...
    TForgeHelper.Release(Tmp); //TtfRecord.Release(Tmp);
    L:= I;
...
//    else R:= @BigNumOne;
  ...
  TmpR:= @BigNumOne;
  Tmp:= A;
  TForgeHelper.Addref(Tmp);  //TtfRecord.AddRef(Tmp);
...
begin
  TForgeHelper.Free(@Self);   //  if @Self <> nil then TtfRecord.Release(@Self);
end;
