...
// -- Or (SetBit) can be used instead of AddNumbersU
  ...
  if Result = TF_S_OK then begin
    TForgeHelper.Free(Ni);
    Ni:= TmpNi;
  end
  else TForgeHelper.Free(TmpNi);

  TForgeHelper.Free(TmpPowerOfTwo);
  TForgeHelper.Free(TmpMask);
  TForgeHelper.Free(TmpProd);
end;
...
// setup Ni
  ...
  if Result = TF_S_OK then begin
    ...
    TForgeHelper.Free(TmpMont);
{   begin
    TForgeHelper.Free(TmpMont.FNi);
    TForgeHelper.Free(TmpMont.FRR);
    TForgeHelper.Free(TmpMont.FN);
  end;}
end;
...
begin
  TForgeHelper.Free(Inst.FNi);
  TForgeHelper.Free(Inst.FRR);
  TForgeHelper.Free(Inst.FN);
...
begin
  ...
  TForgeHelper.Free(Inst.FRR);
  TForgeHelper.Free(Inst.FN);
  BurnSize:= SizeOf(TMontInstance) - Cardinal(@PMontInstance(nil)^.FShift);
  ...
  while Limb > 0 do begin
    if Odd(Limb) then begin
      if TmpR = nil then begin
        ...
        if Result <> TF_S_OK then begin
          TForgeHelper.Free(MontX);
          TForgeHelper.Free(TmpR);
          Exit;
        end;
      end;
